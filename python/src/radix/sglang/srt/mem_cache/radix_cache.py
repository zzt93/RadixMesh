...
#                        token_to_kv_pool_allocator: BaseTokenToKVPoolAllocator, page_size: int, ):
#     """Cache request when it finishes."""
#     if cache.disable:
#         kv_indices = cache.req_to_token_pool.req_to_token[
#                      req.req_pool_idx, : len(req.origin_input_ids) + len(req.output_ids) - 1
#                      ]
#         cache.token_to_kv_pool_allocator.free(kv_indices)
#         cache.req_to_token_pool.free(req.req_pool_idx)
#         return
#
...
#         page_aligned_len = len(kv_indices) // cache.page_size * cache.page_size
#         page_aligned_kv_indices = kv_indices[:page_aligned_len].clone()
#         cache.token_to_kv_pool_allocator.free(kv_indices[page_aligned_len:])
#     else:
...
#
# ### modified ###
# def cache_unfinished_req(cache: RadixCache, req: Req, req_to_token_pool: ReqToTokenPool,
#                          token_to_kv_pool_allocator: BaseTokenToKVPoolAllocator, page_size: int, ):
...
#
#     # Radix Cache takes one ref in memory pool
#     new_prefix_len = cache.insert(page_aligned_token_ids, page_aligned_kv_indices)
#     cache.token_to_kv_pool_allocator.free(
#         kv_indices[len(req.prefix_indices): new_prefix_len]
#     )
#
#     # The prefix indices could be updated, reuse it
#     new_indices, new_last_node, _, _ = cache.match_prefix(page_aligned_token_ids)
