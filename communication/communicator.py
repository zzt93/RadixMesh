...
class Communicator(ABC):
    @abstractmethod
    def send(self, target_id: str, target_ptr: int, length: int) -> int:
        pass
...
class MooncakeCommunicator(Communicator):
    ...
    def __init__(self, hostname: str, length: int, ib_device: Optional[str] = None):
        try:
    ...
    def send(self, target_session_id: str, target_ptr: int, length: int) -> int:
        ...
        try:
            ...
            ret = self.engine.transfer_sync_write(
                target_session_id, self.buffer_addr, target_ptr, length
            )
        except Exception:
